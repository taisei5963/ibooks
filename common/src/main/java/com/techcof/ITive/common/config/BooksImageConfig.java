package com.techcof.ITive.common.config;
 ...
 * ブック画像ファイルURLインタフェース
 */
public interface BooksImageConfig extends ImageConfig {
    /**
     * ディレクトリ階層を取得する
     *
     * @return ディレクトリの階層
    ...
    default Optional<Path> getHierarchyImgDir(Long bookId) {
        ...
        String base = convertHierarchyBase(bookId);
        Path path;
        if (dirHierarchy == 0) {
     ...
     * @return ブック画像URL
     */
    default String getBooksImgUrl(Long bookId, String imgFileName) {
     ...
     */
    private static String convertHierarchyBase(Long bookId) {
