...
import jp.techcof.ITive.common.constant.MapValue;
...
public class CsvEnumValueValidator extends ValidationCellProcessor implements StringCellProcessor {
     ...
     * @param useDescription {@code true} 値の代わりに説明を使用する
     */
    public CsvEnumValueValidator(final Class<? extends Enum<? extends MapValue>> cls,
     ...
     */
    public CsvEnumValueValidator(final Class<? extends Enum<? extends MapValue>> cls,
                                 final boolean useDescription, final CellProcessor next) {
        super(next);
        this.cls = cls;
        this.useDescription = useDescription;
    }
    ...
    public <T> T execute(Object value, CsvContext context) {
        ...
        throw createValidationException(context)
                .messageFormat("列挙型の値が不正です。")
                .messageVariables("values", sj.toString())
                .build();
    }
}
